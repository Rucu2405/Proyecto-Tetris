...
Game::Game()
    : _window(sf::VideoMode(800, 600), "Tetris - Grupo 30")
{
...
void Game::run()
{
    while(_window.isOpen())
    {
        ...
        {
            musicaPartida.musicaPartida.stop();
            procesarEventosMenu();
        }
    ...
    while (_window.pollEvent(evento))
    {
        ...
        case sf::Event::KeyPressed:
            ...
            case sf::Keyboard::Enter:
                efectosMenu.seleccionarOpcion();
                switch (menu.getOpcionElegida())
                {
                case 0:/// iniciar partida
                    musicaMenu.musicaMenu.stop();
                    partidaIniciada = true;
                    irAmenu=false;
                    sf::sleep(sf::seconds(0.2));
                    musicaPartida.fondoPartida();
                    bloque.crearNuevoBloque();
                    break;
                case 1: ///puntajes mas altos
                    irAmenu=false;
                    irApuntos=true;
...
{
    musicaPartida.musicaPartida.stop();
    musicaFinGameOver.fondoGameOver();
    /// crear una pausa
    ...
    musicaFinGameOver.musicaFinGameOver.stop();
    musicaMenu.musicaMenu.play();
}
